...
class AndroidDevMetricsPlugin: Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        // project.dependencies.add("debugImplementation", "org.aspectj:aspectjrt:1.9.7")
        ...
        variants.all { variant ->
            ...
            javaCompiler.doLast {
                val aspectPath = javaCompiler.classpath.asFileTree.filter { !(it.canonicalPath.contains("play-services") && !it.canonicalPath.contains("jetified-play-services")) }.asPath
                val args = arrayOf(
                        "-showWeaveInfo",
                        "-1.8",
                        "-progress",
                ...
                log.info("ajc args: %s".format(args))
                ...
                handler.getMessages(null, true).forEach {
                    when (it.kind) {
                        ...
                        IMessage.WARNING -> log.error(it.message, it.thrown)
